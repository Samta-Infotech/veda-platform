...
## Retrieval upgrade (RETRIEVAL_UPGRADE_PLAN, WP1–WP7) — single-path reality

- **One encoder**: `BAAI/bge-m3` (`ingestion/m3_encoder.py`) produces every 1024-dim dense vector (columns, tables, graph nodes, doc chunks) **and** the learned-sparse weights that replaced BM25. The relgt/light-text/hybrid/MiniLM ensemble and the `ENCODER_MODE` switch are gone; `EMBEDDING_MODEL_ID` stamps the resume guard.
- **Signals**: six-signal **weighted** RRF (`FUSION_WEIGHTS`) — dense, learned-sparse, FK subgraph, FK path, value index, and a **table-first prior** (dense ⊕ sparse table affinity, soft). Graph expansion is **Personalized PageRank** over the source edge list (row-normalized transition matrix), not hop-decay BFS. All engine-local vector tables use **HNSW** with per-source `ef_search`.
- **Precompute is THE path** (WP7): the seven Track-4 consumption flags + `NL_TEMPLATE_ENABLED` were removed. FK signals read only the substrate (`fk_adjacency`) — the query tier makes **zero** source-DB connections outside L7; rerank text is precomputed (fail-loud if the artifact is missing); enrichment/fast-path read the merged/expanded artifacts unconditionally. The genuine resilience fallbacks that remain are **not** flags: Redis→Postgres value resolution, precompiled-then-live join paths, template-then-SLM answers.
