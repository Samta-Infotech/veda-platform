...
def _lane_is_stated(frame: Optional[QueryFrame]) -> bool:
    """Did this frame actually SAY which lane it came from?

    `is_document_frame` answers False both for "this was a table answer" and for "this
    frame never recorded a lane at all", and the guard below must not act on the second:
    silence is not evidence that the conversation was relational.
    """
    if not frame:
        return False
    return (frame.get("entity_is_document") is not None
            or bool(str(frame.get("route") or "").strip()))


# The fields that assert WHAT THE CONVERSATION IS ABOUT, as opposed to what one turn did.
# Restored together: holding `entity` while letting the document turn's `route` /
# `entity_is_document` / scope stand would leave a frame that names a table and reads as a
# document everywhere downstream (is_document_frame prefers entity_is_document).
_SUBJECT_KEYS = ("entity", "entity_display", "route", "entity_is_document",
                 "source_id", "source_ids", "primary_source_id")


def keep_entity_on_lane_change(prev_frame: Optional[QueryFrame], harvested: Dict[str, Any],
                               referential: bool) -> Dict[str, Any]:
    """The mirror of stabilise_document_entity: keep a TABLE conversation on its table
    when the follow-up came back from the documents.

    Measured 2026-09-23: a conversation on `assets_asset` asked "only the Nagpur ones",
    the RAG head answered it ("the provided context does not contain information specific
    to Nagpur", citing an employee handbook), that turn counted as `answered` so memory
    wrote it, and the frame moved to `Samta-Employee Handbook April 2026` — anchoring
    every later turn to a document nobody had asked about.

    Only the claim about what the conversation is ABOUT is refused (_SUBJECT_KEYS, taken
    back from the previous frame; keys the previous frame does not state are dropped
    rather than left stale). What the turn actually ran — its SQL, its row count, its
    datasets — is a fact and is kept.

    Gated on `referential` for the same reason the other guard is: a genuinely new
    question may move to a document, and this must not hold it back. Both lanes must be
    STATED — a frame that recorded no route cannot be protected, because "not a document"
    and "never said" are different things (see _lane_is_stated). The reverse direction (a
    document conversation whose follow-up found a table) is deliberately left alone; it is
    stabilise_document_entity's case, not this one.
    """
    if not referential or not prev_frame or not harvested:
        return harvested
    if not _lane_is_stated(prev_frame) or is_document_frame(prev_frame):
        return harvested                     # no relational lane to protect
    if not is_document_frame(harvested):
        return harvested                     # same lane — nothing changed lanes
    previous = str(prev_frame.get("entity") or "").strip()
    if not previous:
        return harvested                     # nothing to hold
    logger.info("keep_entity_on_lane_change: follow-up was answered from the documents "
                "(%r) — holding the conversation on %r, which is what it is about",
                harvested.get("entity"), previous)
    out = dict(harvested)
