...
def context_resolve_node(state: ChatState, config: RunnableConfig) -> dict:
    ...
    # "no_frame" — which is a statement about what IT could see, NOT about the message.
    # A frame DOES exist by this point (the no-frame turn returned above), and a frame
    # can be fully populated while the IR stack is empty: harvest_entry() yields no entry
    # for an answer that ships no QueryIR, which is every document/RAG turn. Letting that
    # verdict stand there discarded classify_node's real delta_type and reset the
    # conversation on a follow-up the classifier had correctly called a continuation.
    if (_entry is not None and _rule_delta is not None
            and memory_delta.is_confident(_rule_delta)):
        _op = _rule_delta["op"]
