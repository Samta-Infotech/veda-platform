...
| `semantic_search.py` | **Signal 1** — `SemanticSearchEngine.search()`: BGE-M3 dense, raw query, cosine ANN over `column_embeddings_v2` via `storage_adapters.reader.ann_search` (Django HNSW, source-scoped, `VEDA_ANN_VIA_ADAPTER=1`, the default) or the engine's own pgvector (dev/CLI). `DENSE_ID_REMAP` maps UUID `col_id` → `table.column`. |
