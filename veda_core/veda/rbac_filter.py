...
WHY NOT FILTER THE SHARED RETRIEVAL ENGINE ITSELF
    ...
    (``veda_hybrid.py::_load_semantic_model()``) is deliberately NOT pre-filtered:
    the deterministic planners, FK-expansion and the feedback path all need to see
    the full ``sm`` (the feedback path specifically must still recognise a
    restricted-but-real table to say "access denied" rather than "no such table" —
    see ``restricted_names`` and ``veda.feedback``). Enforcement for that head is
    instead the centralized ``narrow_allowed`` gate below, applied to the generated
    SQL's allowlist right before ``validate_and_parameterize``.
...
def restricted_names(sm: dict, ctx) -> dict:
    """Bare table/column names RBAC restricts in ``sm`` for ``ctx``, without
    building a filtered copy of ``sm`` itself (the shared sm is left intact — see
    the module docstring). ``{"tables": [], "columns": []}`` when there is
    nothing to restrict.
...
def filter_nosql_collections(collections, source_id, ctx):
    """The NoSQL schema-narrowing filter (2026-08-08 audit finding: ``_run_nosql``
