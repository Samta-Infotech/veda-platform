...
def qualifier_completeness(query, sql, sm=None, strict=False, user_message=None):
    ...
    if _GATE_STRIP is None:
        _GATE_STRIP = _gate_strip()
    # WHOSE words: the user's own message when the caller passed one (see the docstring)
    # — a chat follow-up arrives here already rewritten with the engine's display label
    # glued on, and those words were never typed by anyone.
    ...
    _raw_toks = re.findall(r"[a-z]+|\d[\d,]*(?:\.\d+)?", (_user_words or "").lower())
    content = set()
    for _w in _raw_toks:
        if _w[0].isdigit():
