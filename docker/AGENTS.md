...
| `pgbouncer.ini` | Transaction pooling `:6432`. `default_pool_size=20`, `reserve_pool_size=5`, `max_prepared_statements=200`, `auth_type=plain`. Comment says "pg16" — **stale (image is pg17)**. |
...
## Gotchas
- **pg17 everywhere except stale comments** (`ARCHITECTURE.md` §11 "pg16" is fixed;
  `pgbouncer.ini` comment is not).
