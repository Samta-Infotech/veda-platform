...
def get_fk_adjacency(table_ids: Optional[List[str]] = None) -> List[FKEdge]:
    ...
    sql = """
        ...
        WHERE e.source_id = ANY(%s) AND e.tenant = %s
    """
    params: list = [source_ids, tenant]
    if table_ids:
...
def ann_search(mode: str, qvec: List[float], top_k: int) -> List[Any]:
    ...
    sql = (
        ...
        f'WHERE source_id = ANY(%s) AND tenant = %s ORDER BY embedding <=> %s::vector LIMIT %s'
    )
    ...
    with _connection().cursor() as cur:
        ...
        cur.execute(sql, [vec, source_ids, tenant, vec, top_k])
        rows = cur.fetchall()
        cur.execute("COMMIT")
        return rows
