...
Runnable two ways:
  - standalone script: ``python tests/test_qualifier_salvage.py`` (from repo root)
  - collected by pytest: ``pytest tests/test_qualifier_salvage.py``
The body is wrapped in test_qualifier_salvage() so the os.chdir()/failure-exit no
longer fire at import time (which crashed ``pytest tests/`` collection); the chdir
is scoped and restored so it never leaks cwd into other tests in the same session.
"""
import json
import os
import sys
