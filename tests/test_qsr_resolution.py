...
Runnable two ways:
  - standalone script: ``python tests/test_qsr_resolution.py`` (from repo root)
  - collected by pytest: ``pytest tests/test_qsr_resolution.py``
The body is wrapped in test_qsr_resolution() so the os.chdir()/failure-exit no
