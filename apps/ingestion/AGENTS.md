...
| File | Role |
|------|------|
| `tasks.py` | `task_ingest_source(source_id, tenant, force, skip_llm, resume, ...)` — creates `IngestionJob` + ordered `IngestionStage` rows; `_guard_embedding_model_change` (`EMBEDDING_MODEL_ID` vs last successful job); injects `Source.as_engine_env()`; `_run_engine_pipeline` → `subprocess.Popen(["python","-u","-c", prog], cwd=veda_core)`; `_build_engine_command` routes by `source.source_kind()` (relational → `main.run_ingestion`; else → `source_dispatcher.dispatch_ingestion`); `_consume_engine_output` parses `[[STAGE]]` events (`_LAYER_STAGE_TO_ROW`) — and a **legacy** `[N/NN]` protocol (`_MARKER_RE`, `_ENGINE_STEP_TO_STAGE`) that the layered path never emits. On success → `task_warm_caches` → `storage_adapters.writer.warm()` → `Source.ready=True`. Flag-gated post-steps: `_sync_catalog_if_enabled` (RBAC catalog), `profile_source_if_enabled`, `build_source_items_if_enabled`. |
